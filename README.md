# Sou o Eduardo 👋

🎯 **Desenvolvedor Fullstack em formação**, com foco em soluções práticas e sistemas reais  
🧠 Apaixonado por **automação, dados, produtividade e organização**  
🔧 Criador do **DATADUH**, um sistema de gestão pessoal e empresarial em evolução
---
## 🚀 Sobre mim
Atualmente estou em uma jornada para me tornar **Fullstack**, transformando experiências reais — como sistemas em Excel VBA em **aplicações web modernas**, acessíveis via navegador e independentes de ferramentas proprietárias.

Aprendo construindo, errando, corrigindo e documentando todo o processo.

---

## 💼 Projeto em destaque

### 📊 DATADUH – Sistema de Gestão Web
ERP pessoal/empresarial com foco em automação e controle:

- 💰 Contas a pagar e controle financeiro  
- 👥 Cadastro de pessoas e clientes  
- 👨‍💼 Folha de pagamento  
- ⚡ Gerenciamento de energia elétrica  
- 🛡️ Controle de apólices de seguro  
- 📦 Gestão de estoque  
- 🗓️ Agenda pessoal  

🔗 Repositório:  
👉 [github.com/DuhhOliveira22/dataduh](https://github.com/DuhhOliveira22/DuhhOliveira22)

---

## 🛠️ Tecnologias em aprendizado e uso

- **Frontend:** HTML, CSS, JavaScript, React  
- **Backend:** Node.js, Express  
- **Banco de Dados:** PostgreSQL, SQLite  
- **Versionamento:** Git & GitHub  
- **Estilo e UI:** Tailwind CSS  
- **Deploy:** Vercel, Render (em estudo)

---

## 📫 Contato

- 📧 Email: **sistemadataduh@gmail.com**  
- 💼 LinkedIn: [Eduardo Oliveira](https://www.linkedin.com/in/eduardo-oliveira-sistema-dataduh)

---

> 💡 *“Automatizar o que é repetitivo é abrir espaço para o que realmente importa.”*
